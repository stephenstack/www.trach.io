                                    ...
                                    <div class="flex items-center w-full">
                                        <div class="flex flex-wrap items-center w-full">
                                            <label class="w-full text-gray-700 tracking-wide text-sm mb-2"
                                                for="desat_value">Enter Desat value</label>
                                                    <input name="desat_value" type="text"
                                                        class="tcl-input"
                                                        value="" v-model="desat_value">
                                                <span v-if="returnedError" class="text-sm font-light text-red-400 tracking-wide" role="alert">
                                                    <strong>{{returnedError}}</strong>
                                                </span>
                                        </div>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        ...
        saveForm() {
            ...
            axios.post('/api/desats', {
                    owner_id: this.$userId,
                    desat_value: this.desat_value,
                ...
                .then((response) => {
                    ...
                    this.$store.commit('SET_DESATSTATUS', response.data.data);
                    this.close();
                })
                .catch((error) => {
                    // console.log(error);
                    this.returnedError = error.response.data.data.desat_value[0];
                });
        },
    },
    mounted(){
