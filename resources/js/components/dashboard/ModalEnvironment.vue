                    ...
                    <div class="bg-white rounded-lg w-full">
                        ...
                        <div class="w-full bg-white rounded-lg mx-auto flex overflow-hidden rounded-b-none">
                            <div class="w-1/3 bg-gray-100 p-8 hidden md:inline-block">
                                <h2 class="font-medium text-md text-gray-700 mb-4 tracking-wide">Environment values</h2>
                                <p class="text-xs text-gray-500">Please record details from Hygrometer display</p>
                            </div>
                                    ...
                                    <div class="flex flex-wrap items-center w-full">
                                        <div class="flex flex-wrap items-center w-full">
                                            <label class="w-full text-gray-700 tracking-wide text-sm mb-2"
                                                for="desat_value">Enter humidity value (%)</label>
                                                    <input name="desat_value" type="text"
                                                        class="tcl-input"
                                                        value="" v-model="humidity">
                                                <span v-if="humidityError" class="text-sm font-light text-red-400 tracking-wide" role="alert">
                                                    <strong>{{humidityError}}</strong>
                                                </span>
                                        </div>
                                        <div class="flex flex-wrap items-center w-full mt-4">
                                            <label class="w-full text-gray-700 tracking-wide text-sm mb-2"
                                                for="desat_value">Enter temperature value ({{this.$store.getters.env_temp_scale.temp_scale_value}})</label>
                                                    <input name="desat_value" type="text"
                                                        class="tcl-input"
                                                        value="" v-model="temperature">
                                                <span v-if="tempError" class="text-sm font-light text-red-400 tracking-wide" role="alert">
                                                    <strong>{{tempError}}</strong>
                                                </span>
                                        </div>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        ...
        saveForm() {
          // console.log(this.desat_value);
          this.humidityError = null;
          this.tempError = null;
            axios.post('/api/envvars', {
                ...
                .then((response) => {
                    ...
                    this.$store.dispatch('SAVE_ENVSSTATUS', response.data.data);
                    // this.sendMessage('success', response.data.message);
                    this.close();
                })
                .catch((error) => {
                  if (error.response.data.data.humidity) { this.humidityError = error.response.data.data.humidity[0]; }
                  if (error.response.data.data.temperature) { this.tempError = error.response.data.data.temperature[0]; }
                });
        },
    },
    mounted(){
