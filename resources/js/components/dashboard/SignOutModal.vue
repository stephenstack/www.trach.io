                ...
                <div class="bg-white rounded-lg w-full">
                        ...
                        <div class="w-full bg-white rounded-lg mx-auto flex overflow-hidden rounded-b-none">
                            <div class="w-1/3 bg-gray-100 p-8 hidden md:inline-block">
                                <h2 class="font-medium text-md text-gray-700 mb-4 tracking-wide">Shift sign-out</h2>
                                <p class="text-xs text-gray-500">Thanks for all your help today! We really appreciate it.</p>
                            </div>
                            ...
                            <div @click="close" class="modal-close flex justify-end cursor-pointer z-50 p-3">
                                ...
                                <div class="pb-8 px-4 lg:px-16">
                                    <div class="items-start w-full">
                                        ...
                                        <p class="w-full"><strong>Sign out Time:</strong> {{datenow}}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="p-6 lg:p-16 py-4 lg:py-8 bg-gray-300 clearfix rounded-b-lg border-t border-gray-200">
                            <p class="hidden md:flex hidden md:flex float-left text-xs text-gray-500 tracking-tight mt-2">Lets get you signed out!</p>
                            <button
                                @click="signOut()"
                                class="bg-indigo-500 text-white text-sm font-medium px-6 py-2 rounded float-right uppercase cursor-pointer hover:shadow-xl">Sign out</button>
                            <button @click="close()"
...
export default {
    ...
    methods: {
        signOut(){
            ...
            axios
                .post('api/shift-sign-out', {
                    current_shift_id: this.$store.getters.shift.id,
                })
                .then(response => {
                    this.$emit('signedOut', response.data.message);
                    this.close();
                })
                .catch(error => {
                     console.log(error)
