                    ...
                    <div class="bg-white rounded-lg w-full">
                        <div class="w-full bg-white rounded-lg mx-auto flex overflow-hidden rounded-b-none">
                            <div class="w-1/3 bg-gray-100 p-8 hidden md:inline-block">
                                <h2 class="font-medium text-md text-gray-700 mb-4 tracking-wide">Add a comment</h2>
                                <p class="text-xs text-gray-500">You can add further details to the note already created.</p>
                            </div>
                            <div class="md:w-full w-full">
                                ...
                                <div class="pb-8 px-4 lg:px-16">
                                    <div class="w-full mt-4">
                                        ...
                                        <span v-if="errors.comment" class="text-sm font-light text-red-400 tracking-wide" role="alert">
                        ...
                        <div class="p-16 py-8 bg-gray-300 clearfix rounded-b-lg border-t border-gray-200">
                            <p class="hidden md:flex float-left text-xs text-gray-500 tracking-tight mt-2">Record a comment!
                            </p>
                            <button
...
export default {
    ...
    methods: {
        onSubmitSave(){
            axios.post('/api/note-comment', {
                    comment: this.comment,
                    note_id: this.parentId,
                    owner_id: this.$userId,
                    owner_name: this.$userName,
                })
                .then((response) => {
                    this.$emit('addrow', response.data[0]);
                    this.reset();
                    this.$emit('close');
                })
                .catch((error) => {
