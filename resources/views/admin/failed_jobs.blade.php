...
@section('content')
    ...
    <div class="p-4 w-full bg-white ml-4 mb-5 rounded">
        <h1 class="text-2xl font-bold text-gray-700 px-6 md:px-0">{{$results->total()}} Failed jobs</h1>
        ...
        <div class="w-full">
            ...
            <div class="bg-white shadow-md rounded my-6">
                <div class="flex justify-end my-4">
                    <a class="bg-red-500 text-white text-sm font-medium px-6 py-2 rounded float-right uppercase cursor-pointer  hover:shadow-xl mr-4" href="/admin/failed-jobs-delete-all">Delete all</a>
                    <a class="bg-indigo-500 text-white text-sm font-medium px-6 py-2 rounded float-right uppercase cursor-pointer  hover:shadow-xl" href="/admin/failed-jobs-retry-all">Retry all</a>
                </div>
                ...
                <thead>
                    <tr>
                        ...
                        <th class="py-4 px-6 bg-gray-300 font-bold uppercase text-sm text-gray-700 border-b border-gray-400">Exception</th>
                        <th class="py-4 px-6 bg-gray-300 font-bold uppercase text-sm text-gray-700 border-b border-gray-400">Failed At</th>
                ...
                <tbody>
                    @foreach ($results as $result)
                        <tr class="hover:bg-gray-200">
                            ...
                            <td class="py-4 px-6 border-b border-gray-400 break-all">{{str_limit($result->exception, 150 )}}</td>
                            <td class="py-4 px-6 border-b border-gray-400">{{ \Carbon\Carbon::parse($result->failed_at)->format('j F, Y') }} </td>
                            <td class="py-4 px-6 border-b border-gray-400 text-right">
                                {{-- <a href="#" class="text-gray-200 font-bold py-1 px-3 rounded text-xs bg-green-600 hover:bg-green-700 ">Edit</a> --}}
