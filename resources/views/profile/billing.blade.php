    ...
    <div class="container mx-auto max-w-5xl mt-8">
        ...
        <form action="{{ route('billing.save') }}" id="billing-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="md:w-2/3 w-full">
                    ...
                    @if(auth()->user()->subscribed('main'))
                        ...
                        <div class="py-8 px-16">
                            ...
                            <div class="flex">
                                <img src="/images/plans/{{ auth()->user()->plan->name }}.png" class="w-16 h-16 mr-3">
                                <div>
                                    <span class="block">Subscribed to <strong>{{ ucfirst(auth()->user()->plan->name) }} Plan</strong></span>
                            ...
                            @else
                                <div class="flex justify-between items-center mt-4">
                                    <div id="switch-plan-btn" class="bg-green-400 hover:bg-green-500 hover:shadow hover:text-white text-white text-sm font-medium px-6 py-2 rounded uppercase cursor-pointer inline-block">
                                        Switch My Plan
                                    </div>
                                    <a href="{{ route('cancel') }}" class="text-red-400 text-sm hover:underline hover:text-red-600">Cancel Subscription</a>
                                </div>
                            @endif
                        </div>
            ...
            <div class="p-16 py-8 bg-gray-300 clearfix rounded-b-lg border-t border-gray-200">
                <p class="hidden md:flex float-left text-xs text-gray-500 tracking-tight mt-2">Click on Save to update your Billing Info</p>
                <button id="card-button" data-secret="{{ auth()->user()->createSetupIntent()->client_secret }}"
                    class="bg-indigo-500 hover:bg-indigo-600 hover:shadow text-white text-sm font-medium px-6 py-2 rounded float-right uppercase cursor-pointer">
